...
# Load packages -----------------------------------------------------------
# Package names
packages <- c( "data.table", "here", "tidyverse", "highcharter", "data.table","revealjs","tools")
...
In your script, include packages that you will be using:
...
- Avoid repetition, use e.g. `for` loops, or `apply` family functions
...
## Read in file from the list
my_data <- file_list2[[1]]
dim(my_data)
...
check_data_cols <- function(real_data, meta_vars){
  ...
  print(text1)
  print(real_names[!(real_names %in% meta_vars)])
  
  print(text2)
  }

check_data_cols(my_data, meta_table$var_name )

meta_var <- meta_table$var_name
my_data<-my_data[,..meta_var]


# Keep only the vars that we chose in the metadata file
my_data_clean<-map2_dfr(my_data,meta_table$missing, recode_to_na )
...
## Choose or create your color palette
  - You use pre-made palettes, e.g. from the `viridis()` package(colorblind friendly!)
  - Or save your own selection of colors in a vector
  - You can inspect the color palette you chose or created with`scales::show_col()`
  
```{r own-colors}
tud_cols <- c('#00A6D6','#FFFFFF', '#000000' )

scales::show_col(tud_cols)
scales::show_col(viridis::viridis(9))
```
